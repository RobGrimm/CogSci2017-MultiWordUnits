...
def segment_corpus_by_decomposition(index_dict, ngram_trie,wild_card_ngram_trie,total_token_count,options,break_trie={}):
    ...
    count_trie = {}
    sent_count = 0
    sents = get_corpus(options.corpus)
    for sentence in sents:
...
def main(options):
    reg_trie, wild_trie, index_dict, freq_cutoff, total_token_count = get_ngram_tries(options)
    ...
    counts_trie, break_trie = decompose_vocabulary(counts_trie, reg_trie, wild_trie, total_token_count, freq_cutoff,
                                                   options)
    if options.output:
